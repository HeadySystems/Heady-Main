...
logger.info('=== Buddy System Tests ===');
...
logger.info('✓ Instantiation: running, zero ingestions');
...
logger.info(`✓ Ingest: id=${r1.id}, X=${r1.coords.x}, Y=${r1.coords.y}, Z=${r1.coords.z}`);
...
logger.info(`✓ Batch ingest: 3 items, total ingested=${buddy.status().ingestionCount}`);
...
logger.info('✓ Spatial ordering: CSS.x > Dockerfile.x, Architecture.z > Dockerfile.z');
...
logger.info(`✓ Context query: ${queryResult.results.length} results, X=${queryResult.coords.x}`);

// ── Nearest retrieval ───────────────────────────────────────
const nearest = buddy.retrieveContext(-0.5, 0.5, 0.2, 2);
console.assert(nearest.length <= 2, 'Nearest should return ≤ 2');
logger.info(`✓ Nearest retrieval: ${nearest.length} items`);
...
logger.info(`✓ Trajectory prediction: X=${predicted.x.toFixed(3)}, Y=${predicted.y.toFixed(3)}, Z=${predicted.z.toFixed(3)}`);
...
(async () => {
    ...
    logger.info(`✓ Pre-fetch: ${prefetchResult.prefetched} blocks pushed to cache`);
    ...
    logger.info(`✓ Temporal decay: pruned=${decayResult.pruned}, remaining=${decayResult.total}`);
    ...
    logger.info(`✓ Cache stats: mode=${cacheStats.mode}, writes=${cacheStats.writes}, hitRate=${cacheStats.hitRate}`);

    // ── Stop ────────────────────────────────────────────────
    buddy.stop();
    console.assert(buddy.status().running === false, 'Should be stopped');
    logger.info('✓ Stop: running=false');

    logger.info('✅ buddy-system: ALL PASS');
})();
