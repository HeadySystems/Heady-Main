...
logger.info('=== Octree Manager Tests ===');
...
logger.info('✓ Insert 5 items, size=5');
...
logger.info(`✓ Range query (backend quadrant): ${backendItems.length} items`);
...
logger.info(`✓ Radius query (r=0.3 near center): ${nearOrigin.length} items, includes shared-config`);
...
logger.info(`✓ Nearest-2 to (-0.5,0.3,0.2): [${nearest.map(n => n.id).join(', ')}]`);
...
logger.info('✓ Remove: size=4, d removed');
...
logger.info('✓ Re-insert: replaced, size unchanged');
...
logger.info(`✓ Stats: items=${stats.totalItems}, maxDepth=${stats.maxDepth}`);

// ── Clear ───────────────────────────────────────────────────
tree.clear();
console.assert(tree.size() === 0, 'Size after clear should be 0');
logger.info('✓ Clear: size=0');
...
logger.info(`✓ Stress test: 100 items inserted, radius query returned ${radiusResults.length}`);

logger.info('✅ octree-manager: ALL PASS');
