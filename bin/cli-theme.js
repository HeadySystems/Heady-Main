...
function printLogo(version = '3.0.0') {
    logger.info(HEADY_LOGO);
    logger.info(`${FG.gray}    ─────────────────────────────────────────${RESET}`);
    logger.info(`${FG.purple}${BOLD}    Heady™ Latent Operating System${RESET}  ${dim(`v${version}`)}`);
    logger.info(SACRED_TAGLINE);
    logger.info('');
}
...
class Spinner {
    ...
    succeed(text) {
        this.stop();
        logger.info(`\r  ${green('✓')} ${text || this._text}`);
    }

    fail(text) {
        this.stop();
        logger.info(`\r  ${red('✗')} ${text || this._text}`);
    }

    warn(text) {
        this.stop();
        logger.info(`\r  ${yellow('⚠')} ${text || this._text}`);
    }
...
function heading(text) {
    logger.info('');
    logger.info(`  ${FG.purple}${BOLD}◆ ${text}${RESET}`);
    logger.info(`  ${FG.gray}${'─'.repeat(stripAnsi(text).length + 2)}${RESET}`);
}

function success(msg) { logger.info(`  ${green('✓')} ${msg}`); }
function info(msg) { logger.info(`  ${azure('ℹ')} ${msg}`); }
function warn(msg) { logger.info(`  ${yellow('⚠')} ${msg}`); }
function errorMsg(msg) { logger.error(`  ${red('✗')} ${msg}`); }
function debug(msg) { logger.info(`  ${dim('⋯ ' + msg)}`); }

function divider(char = '─', width = 50) {
    logger.info(`  ${FG.gray}${char.repeat(width)}${RESET}`);
}

function section(title) {
    logger.info('');
    logger.info(`  ${FG.gold}${BOLD}${title}${RESET}`);
    logger.info(`  ${FG.gray}${'━'.repeat(stripAnsi(title).length)}${RESET}`);
}
...
function separator() {
    ...
    logger.info(`${FG.gray}${'─'.repeat(Math.min(cols, 80))}${RESET}`);
}
